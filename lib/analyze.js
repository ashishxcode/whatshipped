...
export function riskSignals(commits) {
  ...
  if (reverts.length) {
    out.push({
      ...
      detail: `${reverts.length} revert-or-restore commits landed in the window. Each one is a change that shipped and then had to be pulled back — check whether the underlying cause was addressed.`,
  ...
  if (credentials.length) {
    out.push({
      ...
      detail: `${credentials.length} commits touched API keys, quotas or rate limits. Recurring churn here is a standing production risk and usually signals a missing managed-credential strategy.`,
